...
export default function ChatInterface() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("https://n8n-c6s0.onrender.com/webhook-test/flowmate-chat", {
        method: "POST",
        body: formData,
      })
      ...
      const botResponse: Message = {
        id: Date.now().toString(),
        content: responseData.message || "Message received",
        sender: "bot",
        timestamp: new Date(),
      }
      ...
      <Card className="w-full max-w-3xl h-[80vh] flex flex-col">
                    ...
                    {message.files && message.files.length > 0 && (
                      <div className="mt-2 space-y-2">
                        {message.files.map((file) => (
                          <div key={file.id} className="flex items-center bg-background/50 rounded p-2 text-sm">
          ...
          <div className="px-4 py-2 border-t flex flex-wrap gap-2">
            {files.map((file) => (
              <Badge key={file.id} variant="secondary" className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="icon" className="h-4 w-4 ml-1 p-0" onClick={() => removeFile(file.id)}>
                  <X className="h-3 w-3" />
                </Button>
              </Badge>
            ))}
        ...
        <CardFooter className="border-t p-4">
          <form onSubmit={handleSendMessage} className="flex w-full space-x-2">
            ...
            <Button type="submit" disabled={isLoading || (!inputMessage.trim() && files.length === 0)}>
